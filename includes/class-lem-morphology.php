...
class LEM_Morphology {
    ...
    /**
     * Словоформы названия-бренда («Вёрстка» → «Вёрстки», «Вёрстке», «Вёрстку»).
     *
     * Нужно для брендов в кавычках: в статьях их пишут в любом падеже
     * («материал «Вёрстки»»), а кавычки уже гарантируют, что это издание,
     * поэтому склонять безопасно. Несклоняемые типы отдаём как есть.
     *
     * @return string[]
     */
    public static function brand_forms($name) {
        ...
        // Одно слово на -а/-я: Вёрстка, Медуза, Бумага, Сота
        if (preg_match('/^[^\s]+[ая]$/u', $low)) {
            $stem = mb_substr($s, 0, -1);
            $last = mb_strtolower(mb_substr($stem, -1));
            if (mb_substr($low, -1) === 'я') {
                return self::add([$s], $stem, ['и', 'е', 'ю', 'ей']);
            }
            $gen = (mb_strpos(self::HUSHING, $last) !== false) ? 'и' : 'ы';
            return self::add([$s], $stem, [$gen, 'е', 'у', 'ой', 'ою']);
        }
