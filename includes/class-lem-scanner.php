...
class LEM_Scanner {
    ...
    /**
     * Слова, из которых состоят обычные фразы. Если алиас собран ТОЛЬКО из
     * них («Служба поддержки», «Латвийская Республика»), искать его без кавычек
     * нельзя: в реестре это часть названия, а в тексте статьи обычный оборот.
     */
    const GENERIC_WORDS = [
        'служба', 'службы', 'службу', 'поддержка', 'поддержки', 'поддержку',
        'помощь', 'помощи', 'фонд', 'фонда', 'центр', 'центра', 'союз',
        'организация', 'организации', 'общество', 'общества', 'группа', 'группы',
        'институт', 'комитет', 'совет', 'ассоциация', 'объединение', 'инициатива',
        'платформа', 'форум', 'клуб', 'школа', 'университет', 'академия',
        'лаборатория', 'мастерская', 'редакция', 'издание', 'издания', 'газета',
        'журнал', 'портал', 'канал', 'сайт', 'новости', 'информация',
        'информационный', 'информационное', 'право', 'права', 'прав', 'правам',
        'человека', 'людей', 'защита', 'защиты', 'защите', 'защиту',
        'республика', 'республики', 'федерация', 'федерации',
        'латвийская', 'литовская', 'эстонская', 'немецкая', 'российская',
        'российский', 'украинская', 'американская', 'чешская', 'польская',
        'международная', 'международный', 'международное',
        'общественная', 'общественное', 'общественный',
        'независимая', 'независимый', 'независимое',
    ...
    const STOP_WORDS = ['и', 'в', 'на', 'по', 'для', 'о', 'об', 'с', 'за', 'из', 'от', 'the', 'of'];

    /**
     * Фраза целиком собрана из общеупотребительных слов?
     * Такие алиасы ищем только в кавычках.
     */
    public static function is_generic_phrase($term) {
        // trim() с многобайтовым списком символов режет байты и портит буквы
        // («центр» -> «цент»), поэтому чистим края регуляркой
        $strip = static function ($w) {
            return preg_replace('/^[«»"„“”\'().,;:]+|[«»"„“”\'().,;:]+$/u', '', $w);
        };
        $words = preg_split('/[\s\-–—]+/u', mb_strtolower(trim((string) $term)), -1, PREG_SPLIT_NO_EMPTY);
        $words = array_values(array_filter(array_map($strip, $words), static function ($w) {
            return $w !== '' && !in_array($w, self::STOP_WORDS, true);
        }));
        // Одиночные слова отсекает TERM_BLACKLIST. Длинные фразы из четырёх и
        // более слов случайными в тексте не бывают: «Международная федерация за
        // права человека» это уже конкретное название, а не оборот речи.
    ...
    /**
     * Алиасы, заданные пользователем в разделе «Бренды СМИ», проверке на
     * общеупотребительность не подлежат: их выбрал человек осознанно.
     */
    private static function curated_aliases() {
        static $set = null;
        if ($set !== null) {
            return $set;
        }
        $set = [];
        if (function_exists('lem') && isset(lem()->brands)) {
            foreach (lem()->brands->get_rules() as $rule) {
                if (empty($rule['enabled'])) {
                    continue;
                }
                foreach (array_merge($rule['aliases'], $rule['quoted']) as $a) {
                    $set[mb_strtolower(trim($a))] = true;
                }
            }
        }
        return $set;
